...
ppl_entry #(
 ...
 u_ppl_entry (
    ...
    .rst                     ( rst                                            ),
    .enable                  ( enable                                         ),
    .p_pos_x                 ( p_pos_x              [31:0]                    ),
    .p_pos_y                 ( p_pos_y              [31:0]                    ),
    .p_pos_z                 ( p_pos_z              [31:0]                    ),
    .p_angle_x               ( p_angle_x            [31:0]                    ),
    .p_angle_y               ( p_angle_y            [31:0]                    ),
    .next_pixel_pplout       ( next_pixel_pplout                              ),
    .pixel_addr_pplout       ( pixel_addr_pplout    [DISP_RAM_ADDR_RADIX-1:0] ),
    .to_color_acc_pplout     ( to_color_acc_pplout  [31:0]                    ),
    .to_block_p_pplout       ( to_block_p_pplout    [2:0]                     ),
    .to_dir_pplout           ( to_dir_pplout        [1:0]                     ),
    .to_hit_p_pplout         ( to_hit_p_pplout      [2:0]                     ),
    .start_p_pplout          ( start_p_pplout       [2:0]                     ),
    .end_p_pplout            ( end_p_pplout         [2:0]                     ),
    .to_is_behind_pplout     ( to_is_behind_pplout                            ),

    .is_preparing            ( is_preparing                                   ),
    .is_eof                  ( is_eof                                         ),
    .towards_h               ( towards_h            [1:0]                     ),
    .idle                    ( idle                                           ),
    .pixel_addr              ( pixel_addr           [DISP_RAM_ADDR_RADIX-1:0] ),
    .color_acc               ( color_acc            [31:0]                    ),
    .from_dir                ( from_dir             [1:0]                     ),
    .block_p                 ( block_p              [2:0]                     ),
    .hit_p                   ( hit_p                [2:0]                     ),
    .start_p                 ( start_p              [2:0]                     ),
    .end_p                   ( end_p                [2:0]                     ),
